...
- (void)viewDidLoad
{
    ...
    if(![SNAccount haveToken]) {
        [ErrorView showError:@"请登录账号,否则将处于宾客模式,无法远程控制您的设备!" withShowDuration:2.5];
    }
}
...
//        }
        ...
        if (retDataValue == 0)  // 关机
        {
            self.switchBtn.selected = NO;
            [self.switchBtn setTitle:@"OFF" forState:UIControlStateSelected];
            self.switchBtn.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 35.0);
            self.switchBtn.titleEdgeInsets = UIEdgeInsetsMake(0, -image.size.width/2, 0, 0);
        }
        else if(retDataValue == 1)// 开机
        {
            self.switchBtn.selected = YES;
            [self.switchBtn setTitle:@"ON" forState:UIControlStateNormal];
            self.switchBtn.imageEdgeInsets = UIEdgeInsetsMake(0, 35, 0, 0);
            self.switchBtn.titleEdgeInsets = UIEdgeInsetsMake(0, -image.size.width/2-25, 0, image.size.width/2);
        }
    }
...
//    [self.switchBtn setBackgroundImage:[UIImage imageNamed:@"switch_on"] forState:UIControlStateSelected];
    [self.switchBtn setBackgroundImage:[UIImage imageNamed:@"switch_off"] forState:UIControlStateSelected];
    [self.switchBtn setBackgroundImage:[UIImage imageNamed:@"switch_on"] forState:UIControlStateNormal];
